...
描述: Dubstep
        ...
        Recently, he has decided to take a couple of old
        ...
        Let's assume that a song consists of some number
         ...
         before the first word of the song (the number may
         be zero), after the last word (the number may be
         zero), and between words (at least one between any
         pair of neighbouring words), and then the boy glues
         together all the words, including "WUB", in one
         string and plays the song at the club.

        For example, a song with words "I AM X" can
        transform into a dubstep remix as "WUBWUBIWUBAMWUBWUBX"
        ...
        used to make a dubsteb remix. Separate the words
        with a space.
...
def song_decoder(song):
    song = song.replace("WUB"," ").replace("  "," ")
    if "  " in song:
        return song_decoder(song)
    else:
        return song.strip()
